...
def main():
    ...
    issues = []
    print(f'=== HUMAN AUDIT: {os.path.basename(ckpt)} ===')
    ...
    print(f'  {tag(bool(l0_mouse_holds))} L0 direct @L{ml}:hold: {[(p,e,h,"T" if t else "F") for p,e,h,t in l0_mouse_holds]}')
    print()
    ...
    if nav_layers:
        dominant = max(nav_layers, key=lambda l: len(nav_layers[l]))
        ...
        print(f'  {tag3(coverage >= 4, coverage >= 3)} Nav cluster: L{dominant} has {coverage}/{len(NAV_CLUSTER)} nav keys')
    ...
    for keys, positions in key_positions.items():
        ...
        lyrs = sorted(set(l for _, l, _, _, _ in positions))
    ...
    print('--- 5. LAYER DEPTH ---')
    deep = [(l, da[l], dh.get(l, 999)) for l in da if l != 0 and l != 7 and da[l] > 2]
    print(f'  {tag(not deep)} Layers deeper than 2 hops: {deep or "none"}')
    if deep: issues.append(f'Layers at depth >2: {[l for l,_,_ in deep]}')
    print()
    ...
    print(f'  {tag3(thumb_switch_ratio > 0.5, thumb_switch_ratio > 0.3)} Thumb cluster: {thumb_layer_switches}/{thumb_total} are layer switches, {thumb_empty} empty')
    ...
    if bad_thumb:
        print(f'  [WARN] High-effort (≥1.5) thumb layer switches: {len(bad_thumb)} (acceptable for rare layers)')
    print()
